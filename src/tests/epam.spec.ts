...
test.describe('Epam', {
  ...
  test('[ @epam-1 ] Check the title is correct', async ({page, epamPage}) => {
  ...
  test('[ @epam-2 ] Check the ability to switch Light / Dark mode', async ({page, epamPage}) => {
    await test.step('Open EPAM.com', async () => {
      await epamPage.open();
      await expect(page).toHaveURL(epamPage.getPageUrl());
    });
  ...
  test('[ @epam-3 ] Check that allow to change language to UA', async ({page, epamPage}) => {
    await test.step('Open EPAM.com', async () => {
      await epamPage.open();
      await expect(page).toHaveURL(epamPage.getPageUrl());
    });
    ...
    await test.step('Switch the site\'s language to Ukraine', async () => {
      ...
      const updatedText = await epamPage.header.getUpdatedLocationSelectorName();
      expect(updatedText).toBe('Україна (UA)');
    })
  })

  test('[ @epam-4 ] Check the policies list', async ({page, epamPage}) => {
    await test.step('Open EPAM.com', async () => {
      await epamPage.open();
      await expect(page).toHaveURL(epamPage.getPageUrl());
    });
  ...
  test('[ @epam-5 ] Check that allow to switch location list by region', async ({page, epamPage}) => {
    await test.step('Open EPAM.com', async () => {
      await epamPage.open();
      await expect(page).toHaveURL(epamPage.getPageUrl());
    });
